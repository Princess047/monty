#include "monty.h"
/**
 * push_handler - adds a new node at the beginning of a dlistint_t list
...
void push_handler(stack_t **head, unsigned int line_number)
{
        ...
        if (helpy.queueflag == 1)
                result = queue_push(&helpy.head, helpy.line_number);
        if (result == 0)
                return;
        if (!head)
        {
...
/**
 * pall_handler - prints all the elements of a stack_t list
...
/**
 * pint_handler - prints first element of a dlistint_t list
 * @head: pointer to a pointer to a struct of type stack_t
 * @line_number: line number of instruction
 * Return: void
 */
